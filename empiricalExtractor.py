...
chat = ChatOpenAI(openai_api_key="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", temperature=0)
...
#def LlamaModel(system, message):
#    return llamaChain.run(system,message)
...
def prepare_text(data_paper):
    # Document folder
    file_path = "SData/documents/" + data_paper['primary_location.pdf_url'].rsplit('/', 1)[-1]
    ...
    if (article_dict is not None):
        ...
        for figure in article_dict['figures']:
            if (figure['figure_type'] == 'table'):
                figures = figures + "In table " + figure['figure_label'] +' of the document we can see: '+ figure['figure_caption'] + " \n\n "
            else:
                figures = figures + "In figure " + figure['figure_label'] +' of the document we can see: '+ figure['figure_caption'] + " \n\n "
...
def preprocess(data_paper):
    ...
    if (finaltext != 'error'):
        if os.path.isfile("./vectors/"+id+"/index.faiss"):
            ...
            # Load the index
            docsearch = FAISS.load_local("./vectors/"+id,embeddings=embeddings)
        else:
...
def collection(docsearch, uses):    
    ...
    query = """Which of the following types best matches the collection process mentioned in the context?
        Types: Web API, Web Scrapping, Sensors, Manual Human Curator, Software collection, Surveys, Observations, Interviews, Focus groups, Document analysis, Secondary data analysis, Physical data collection, Self-reporting, Experiments, Direct measurement, Document analysis, Customer feedback data, Audio or video recordings, Image data, User-generated content data.
...
def annotation(docsearch): 
    ...
    if(classifications['labels'][0] == 'Yes'):
        ...
        query = '''Which  of the following category corresponds to the annotation process mentioned in the context? 
                Categories: Bounding boxes, Lines and splines, Semantinc Segmentation, 3D cuboids, Polygonal segmentation, image Landmark and key-point, text Entity Annotation, Text Categorization, Audio Transcription, Speakers Identification, Anomaly Detection, Music Genre Classification, audio Pitch Annotation, Pose Estimation, video Object Tracking, Action Recognition, Event Annotation, Temporal Annotation, Emotion Recognition, State Labeling, Temporal Pattern Recognition
        ...
        ## Type of the team
        docs = clean_text(docsearch.similarity_search("The team who annotated the data", k=4))
        system = 'Use the following pieces of context to answer the question. Please, answer only with internal team, external team, or crowdsourcing team'
        query = 'The data was annotated by an internal team (e.g the authors), an external team, or a crowdsourcing team?'
        message = 'Question:'+query+' \n ### \n Context: '+ annotation["annotation_explanation"] +' \n\n'+ docs+' ### Answer:'
        ...
        query = """The context provides information about the validation methods used to done to validate the labels?"""
        docs = clean_text(docsearch.similarity_search("Validation methods applied to the labels", k=4))
        message = 'Question:'+query+' \n ### \n Context: '+ annotation["annotation_explanation"] +' \n\n '+ docs+' ### Answer :'
        ...
        if('Yes' or classifications['labels'][0] == 'Yes'):
            ...
            annotation['annotation_validation_methods_types'] = LanguageModel(system=system,message=message).content
...
df = pd.read_csv('SData/RemainingDataPapersSData.csv')


#ids = ["s41597-022-01855-9", "sdata.2018.25", "sdata.2018.161", "s41597-021-00815-z" , "s41597-022-01867-5", "s41597-022-01916-z", "s41597-022-01905-2", "s41597-022-01809-1"]

with open('ResultsRemaininPapers.csv', 'w', newline='') as output_file:
    results = []
    for index, data_paper in df.iterrows():
        print("Processing the:"+ data_paper['display_name'])
        
        if index > 781:
            #tags = get_tags(data_paper)
            tags = ""
            try:
            ...
            output_path = "SData/ResultsRemaininPapers.csv"
